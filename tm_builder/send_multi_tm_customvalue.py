...
sim_state = {
    ...
    "thruster_status": 0,
    "custom_value": 0.0,    # nilai yang di-set lewat command 4-byte float
}
...
def listen_for_tc():
    """UDP TC handler: decode Enable/DisableThruster (1-byte) & SetCustomValue (4-byte)."""
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    while True:
        ...
        # payload length = length field + 1
        payload_len = length + 1
        payload = data[6:6 + payload_len]

        # Thruster command: 1-byte payload
        if payload_len == 1:
            thr_arg = payload[0]
        ...
        # Custom value command: 4-byte float payload
        elif payload_len == 4:

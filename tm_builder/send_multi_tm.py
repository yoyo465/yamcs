...
def listen_for_tc():
    ...
    while True:
        ...
        # Enable/DisableThruster → 1 byte
        if len(payload) == 1:
            thr_arg = payload[0]
            sim_state["thruster_status"] = 1 if thr_arg != 0 else 0
            print(f"[SIM] TC Thruster → status={'ON' if thr_arg else 'OFF'}")
        # SetCustomValue → 4 byte float
        elif len(payload) == 4:
